...
import {
  aiBlock3Lessons,
  aiBlock3LessonsByNumber,
} from "@/data/professions/ai/lessons-block-3";
import { validateAccessTokenForPrograms } from "@/lib/course-access";
import { appendToken, aiBlock3AccessKeys } from "../access";
...
export function getAiBlock3LessonMetadata(lessonNumber: number): Metadata {
  const lesson = aiBlock3LessonsByNumber[lessonNumber];
...
export async function AiBlock3LessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = aiBlock3LessonsByNumber[lessonNumber];
  const { token } = await searchParams;
  const access = validateAccessTokenForPrograms(token, aiBlock3AccessKeys, { requiredBlock: 3 });
  ...
  const sortedLessons = [...aiBlock3Lessons].sort(
  ...
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? appendToken(`/course/ai/basic/block-3/lesson-${lesson.lessonNumber - 1}`, token)
      : appendToken("/course/ai/basic/block-2/lesson-10", token);
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? appendToken(`/course/ai/basic/block-3/lesson-${lesson.lessonNumber + 1}`, token)
      : appendToken("/course/ai/basic/block-3-test", token);
  const nextLessonLabel =
    lesson.lessonNumber < lesson.totalLessons
      ? undefined
      : "Перейти к тесту Блока 3";
  const lessonNavigation = sortedLessons.map((item) => ({
    href: appendToken(`/course/ai/basic/block-3/lesson-${item.lessonNumber}`, token),
