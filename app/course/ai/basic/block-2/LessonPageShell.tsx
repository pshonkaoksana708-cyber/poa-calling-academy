...
import {
  aiBlock2Lessons,
  aiBlock2LessonsByNumber,
} from "@/data/professions/ai/lessons-block-2";
import { validateAccessTokenForPrograms } from "@/lib/course-access";
import { appendToken, getAiAccessPlan, aiBlock2AccessKeys } from "../access";
...
export function getAiBlock2LessonMetadata(lessonNumber: number): Metadata {
  const lesson = aiBlock2LessonsByNumber[lessonNumber];
  ...
  return {
    ...
    description: lesson
      ? `Урок ${lesson.lessonNumber} практического уровня образовательной программы «Специалист по искусственному интеллекту».`
      : undefined,
...
export async function AiBlock2LessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = aiBlock2LessonsByNumber[lessonNumber];
  const { token } = await searchParams;
  const access = validateAccessTokenForPrograms(token, aiBlock2AccessKeys, { requiredBlock: 2 });
  ...
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? appendToken(`/course/ai/basic/block-2/lesson-${lesson.lessonNumber - 1}`, token)
      : appendToken("/course/ai/basic/lesson-10", token);
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? appendToken(`/course/ai/basic/block-2/lesson-${lesson.lessonNumber + 1}`, token)
      : appendToken("/course/ai/basic/block-2-test", token);
  const nextLessonLabel =
    lesson.lessonNumber < lesson.totalLessons
      ? undefined
      : "Перейти к тесту Блока 2";
  const lessonNavigation = [...aiBlock2Lessons]
    .sort((first, second) => first.lessonNumber - second.lessonNumber)
    .map((item) => ({
      href: appendToken(`/course/ai/basic/block-2/lesson-${item.lessonNumber}`, token),
