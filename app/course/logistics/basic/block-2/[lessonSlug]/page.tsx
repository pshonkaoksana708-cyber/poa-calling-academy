import {
  getLogisticsBlock2LessonMetadata,
  LogisticsBlock2LessonPage,
...
export async function generateMetadata({ params }: LessonRouteProps) {
  ...
  return getLogisticsBlock2LessonMetadata(getLessonNumber(lessonSlug));
}

export default async function LogisticsBlock2LessonRoutePage({
  params,
  searchParams,
}: LessonRouteProps) {
  const { lessonSlug } = await params;
