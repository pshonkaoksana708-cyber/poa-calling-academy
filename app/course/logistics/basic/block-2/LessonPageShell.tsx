...
import {
  logisticsBlock2Lessons,
  logisticsBlock2LessonsByNumber,
} from "@/data/professions/logistics/lessons-block-2";
import { validateAccessTokenForPrograms } from "@/lib/course-access";
import {
  appendToken,
  getLogisticsAccessPlan,
  getLogisticsAccessToken,
  logisticsBlock2AccessKeys,
} from "../access";
...
export function getLogisticsBlock2LessonMetadata(lessonNumber: number): Metadata {
  const lesson = logisticsBlock2LessonsByNumber[lessonNumber];
...
export async function LogisticsBlock2LessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = logisticsBlock2LessonsByNumber[lessonNumber];
  ...
  const access = validateAccessTokenForPrograms(token, logisticsBlock2AccessKeys, { requiredBlock: 2 });
  ...
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? appendToken(`/course/logistics/basic/block-2/lesson-${lesson.lessonNumber - 1}`, token)
      : appendToken("/course/logistics/basic/lesson-10", token);
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? appendToken(`/course/logistics/basic/block-2/lesson-${lesson.lessonNumber + 1}`, token)
      : appendToken("/course/logistics/basic/block-2-test", token);
  const nextLessonLabel =
    lesson.lessonNumber < lesson.totalLessons
      ? undefined
      : "Перейти к тесту Блока 2";
  const lessonNavigation = [...logisticsBlock2Lessons]
    .sort((first, second) => first.lessonNumber - second.lessonNumber)
    .map((item) => ({
      href: appendToken(`/course/logistics/basic/block-2/lesson-${item.lessonNumber}`, token),
