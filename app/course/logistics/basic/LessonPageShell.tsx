...
import {
  logisticsBlock1Lessons,
  logisticsBlock1LessonsByNumber,
} from "@/data/professions/logistics/lessons-block-1";
import { validateAccessTokenForPrograms } from "@/lib/course-access";
import {
  appendToken,
  getLogisticsAccessPlan,
  getLogisticsAccessToken,
  logisticsBlock1AccessKeys,
} from "./access";
...
export function getLogisticsBlock1LessonMetadata(lessonNumber: number): Metadata {
  const lesson = logisticsBlock1LessonsByNumber[lessonNumber];
  ...
  return {
    ...
    description: lesson
      ? `Урок ${lesson.lessonNumber} базового уровня образовательной программы «Специалист по международной логистике».`
      : undefined,
...
export async function LogisticsBlock1LessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = logisticsBlock1LessonsByNumber[lessonNumber];
  ...
  const access = validateAccessTokenForPrograms(token, logisticsBlock1AccessKeys, { requiredBlock: 1 });
  ...
  const accessPlan = access.ok ? getLogisticsAccessPlan(access.payload) : getLogisticsAccessPlan();
  const displayTotalLessons =
    accessPlan === "basic" ? 10 : accessPlan === "practice" ? 20 : 30;
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? appendToken(`/course/logistics/basic/lesson-${lesson.lessonNumber - 1}`, token)
      : undefined;
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? appendToken(`/course/logistics/basic/lesson-${lesson.lessonNumber + 1}`, token)
      : appendToken("/course/logistics/basic/block-1-test", token);
  const nextLessonLabel =
    lesson.lessonNumber < lesson.totalLessons
      ? undefined
      : "Перейти к тесту Блока 1";
  const lessonNavigation = [...logisticsBlock1Lessons]
    .sort((first, second) => first.lessonNumber - second.lessonNumber)
    .map((item) => ({
      href: appendToken(`/course/logistics/basic/lesson-${item.lessonNumber}`, token),
  ...
  return (
    <LessonExperience
      backHref={appendToken("/course/logistics/basic", token)}
