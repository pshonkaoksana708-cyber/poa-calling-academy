...
import {
  logisticsBlock3Lessons,
  logisticsBlock3LessonsByNumber,
} from "@/data/professions/logistics/lessons-block-3";
...
export function getLogisticsBlock3LessonMetadata(lessonNumber: number): Metadata {
  const lesson = logisticsBlock3LessonsByNumber[lessonNumber];
  ...
  return {
    ...
    description: lesson
      ? `Урок ${lesson.lessonNumber} профессионального уровня образовательной программы «Специалист по международной логистике».`
      : undefined,
...
export async function LogisticsBlock3LessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = logisticsBlock3LessonsByNumber[lessonNumber];
  ...
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? appendToken(`/course/logistics/basic/block-3/lesson-${lesson.lessonNumber - 1}`, token)
      : appendToken("/course/logistics/basic/block-2/lesson-10", token);
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? appendToken(`/course/logistics/basic/block-3/lesson-${lesson.lessonNumber + 1}`, token)
      : appendToken("/course/logistics/basic/block-3-test", token);
  ...
  const lessonNavigation = [...logisticsBlock3Lessons]
    .sort((first, second) => first.lessonNumber - second.lessonNumber)
    .map((item) => ({
      href: appendToken(`/course/logistics/basic/block-3/lesson-${item.lessonNumber}`, token),
