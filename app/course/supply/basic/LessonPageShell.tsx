...
export function getSupplyBasicLessonMetadata(lessonNumber: number): Metadata {
  ...
  return {
    ...
    description: lesson
      ? `Урок ${lesson.lessonNumber} базового уровня образовательной программы «Специалист по снабжению».`
      : undefined,
  };
}
...
export async function SupplyBasicLessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = supplyBasicLessonsByNumber[lessonNumber];
  ...
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? withToken(`/course/supply/basic/lesson-${lesson.lessonNumber - 1}`, token)
      : undefined;
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? withToken(`/course/supply/basic/lesson-${lesson.lessonNumber + 1}`, token)
      : withToken("/course/supply/basic/block-1-test", token);
  const nextLessonLabel =
    lesson.lessonNumber < lesson.totalLessons
      ? undefined
      : "Перейти к тесту Блока 1";
  const lessonNavigation = [...supplyBasicLessons]
    .sort((first, second) => first.lessonNumber - second.lessonNumber)
    .map((item) => ({
      href: withToken(`/course/supply/basic/lesson-${item.lessonNumber}`, token),
