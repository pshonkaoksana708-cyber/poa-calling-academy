...
import {
  hrBlock1Lessons,
  hrBlock1LessonsByNumber,
} from "@/data/professions/hr/lessons-block-1";
...
export function getHrBlock1LessonMetadata(lessonNumber: number): Metadata {
  const lesson = hrBlock1LessonsByNumber[lessonNumber];
...
export async function HrBlock1LessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = hrBlock1LessonsByNumber[lessonNumber];
  const { token } = await searchParams;
  const access = validateAccessTokenForPrograms(token, hrBlock1AccessKeys, { requiredBlock: 1 });
  ...
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? appendToken(`/course/hr/basic/lesson-${lesson.lessonNumber - 1}`, token)
      : undefined;
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? appendToken(`/course/hr/basic/lesson-${lesson.lessonNumber + 1}`, token)
      : appendToken("/course/hr/basic/block-1-test", token);
  const nextLessonLabel =
    lesson.lessonNumber < lesson.totalLessons
      ? undefined
      : "Перейти к тесту Блока 1";
  const lessonNavigation = [...hrBlock1Lessons]
    .sort((first, second) => first.lessonNumber - second.lessonNumber)
    .map((item) => ({
      href: appendToken(`/course/hr/basic/lesson-${item.lessonNumber}`, token),
