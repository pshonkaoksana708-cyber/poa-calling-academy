...
import {
  hrBlock3Lessons,
  hrBlock3LessonsByNumber,
} from "@/data/professions/hr/lessons-block-3";
...
export function getHrBlock3LessonMetadata(lessonNumber: number): Metadata {
  ...
  return {
    ...
    description: lesson
      ? `Урок ${lesson.lessonNumber} профессионального уровня образовательной программы «Специалист по кадрам и управлению персоналом».`
      : undefined,
  };
}
...
export async function HrBlock3LessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = hrBlock3LessonsByNumber[lessonNumber];
  ...
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? appendToken(`/course/hr/basic/block-3/lesson-${lesson.lessonNumber - 1}`, token)
      : appendToken("/course/hr/basic/block-2/lesson-10", token);
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? appendToken(`/course/hr/basic/block-3/lesson-${lesson.lessonNumber + 1}`, token)
      : appendToken("/course/hr/basic/block-3-test", token);
  const nextLessonLabel =
    lesson.lessonNumber < lesson.totalLessons
      ? undefined
      : "Перейти к тесту Блока 3";
  const completionHref = appendToken(
    "/course/hr/basic/completed?package=professional",
    token,
  );
  ...
  const lessonNavigation = [...hrBlock3Lessons]
    .sort((first, second) => first.lessonNumber - second.lessonNumber)
    .map((item) => ({
      href: appendToken(`/course/hr/basic/block-3/lesson-${item.lessonNumber}`, token),
