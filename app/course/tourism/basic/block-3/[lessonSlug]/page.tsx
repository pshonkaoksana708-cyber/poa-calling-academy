import {
  getTourismBlock3LessonMetadata,
  TourismBlock3LessonPage,
...
export async function generateMetadata({ params }: LessonRouteProps) {
  ...
  return getTourismBlock3LessonMetadata(getLessonNumber(lessonSlug));
}

export default async function TourismBlock3LessonRoutePage({
  params,
  searchParams,
}: LessonRouteProps) {
  const { lessonSlug } = await params;
