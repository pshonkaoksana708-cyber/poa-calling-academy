...
import {
  tourismBlock2Lessons,
  tourismBlock2LessonsByNumber,
} from "@/data/professions/tourism/lessons-block-2";
import { validateAccessTokenForPrograms } from "@/lib/course-access";
import {
  appendToken,
  getTourismAccessPlan,
  tourismBlock2AccessKeys,
} from "../access";
...
export function getTourismBlock2LessonMetadata(lessonNumber: number): Metadata {
  const lesson = tourismBlock2LessonsByNumber[lessonNumber];
  ...
  return {
    ...
    description: lesson
      ? `Урок ${lesson.lessonNumber} практического уровня образовательной программы «Специалист по туризму».`
      : undefined,
...
export async function TourismBlock2LessonPage({
  lessonNumber,
  searchParams,
}: LessonPageProps) {
  const lesson = tourismBlock2LessonsByNumber[lessonNumber];
  const { token } = await searchParams;
  const access = validateAccessTokenForPrograms(token, tourismBlock2AccessKeys, { requiredBlock: 2 });
  ...
  const previousLessonHref =
    lesson.lessonNumber > 1
      ? appendToken(`/course/tourism/basic/block-2/lesson-${lesson.lessonNumber - 1}`, token)
      : appendToken("/course/tourism/basic/lesson-10", token);
  const nextLessonHref =
    lesson.lessonNumber < lesson.totalLessons
      ? appendToken(`/course/tourism/basic/block-2/lesson-${lesson.lessonNumber + 1}`, token)
      : appendToken("/course/tourism/basic/block-2/test", token);
  const nextLessonLabel =
    lesson.lessonNumber < lesson.totalLessons
      ? undefined
      : "Перейти к тесту Блока 2";
  const lessonNavigation = [...tourismBlock2Lessons]
    .sort((first, second) => first.lessonNumber - second.lessonNumber)
    .map((item) => ({
      href: appendToken(`/course/tourism/basic/block-2/lesson-${item.lessonNumber}`, token),
